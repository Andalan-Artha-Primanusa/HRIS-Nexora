...
const AttendanceOverviewPage = () => {
  ...
  const isPresentRecord = (record: Record<string, unknown>) => {
    ...
    if (status.includes('present') || status.includes('hadir') || status.includes('approved')) {
      return true;
    }
    if (status.includes('absent') || status.includes('tidak')) {
      return false;
    }
  ...
  const calcWorkMinutes = (record: Record<string, unknown>) => {
    const explicitMinutes =
      getNumericValue(record.work_minutes) ??
      getNumericValue(record.workHours) ??
      getNumericValue(record.work_hours) ??
      getNumericValue(record.total_minutes);
  ...
  useEffect(() => {
    const loadAttendanceOverview = async () => {
      ...
      try {
        ...
        const todayRecord = todayPayload.data && typeof todayPayload.data === 'object'
          ? toRecord(todayPayload.data)
          : todayPayload;
        ...
        const todayAttendanceRate = getNumericValue(
          todayRecord.attendance_rate ?? todayRecord.rate ?? todayRecord.present_rate
        ) ?? 0;
  ...
  const summaryCards = [
    {
      ...
      subtitle: 'Persentase kehadiran bulan ini',
    ...
    {
      label: 'Hari Hadir',
      subtitle: 'Total hari kehadiran',
      value: `${attendanceStats.present}/${attendanceStats.totalWorkingDays}`,
      tone: 'green',
      icon: Calendar,
    },
    {
      label: 'Hari Tidak Hadir',
      subtitle: 'Total hari tidak hadir',
    ...
    },
    {
      ...
      value: `${attendanceStats.workHours}h`,
      tone: 'purple',
      icon: Clock,
    },
  ];
  ...
  return (
    <div className="attendance-page">
      <Card className="attendance-hero-card" glass>
        <div className="attendance-hero-copy">
          ...
          <p className="attendance-hero-copy-text">
      ...
      <div className="attendance-summary-grid">
        {summaryCards.map((card) => {
          ...
          return (
            <Card key={card.label} className="attendance-summary-card" glass>
              <div className="attendance-summary-head">
                <div>
                  <p className="summary-label">{card.label}</p>
                  <p className="summary-subtitle">{card.subtitle}</p>
                </div>
                <span className={`attendance-icon attendance-icon--${card.tone}`}>
                  <Icon size={18} />
                </span>
              </div>
              <div className={`attendance-summary-value attendance-summary-value--${card.tone}`}>
                {loading ? '...' : card.value}
              </div>
            </Card>
          );
